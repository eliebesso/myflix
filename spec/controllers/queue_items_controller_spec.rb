...
describe QueueItemsController do
	...
	describe "DELETE destroy" do
		...
		it "normalizes the remaining queue items" do
			josias = Fabricate(:user)
			set_current_user(josias)
			queue_item1 = Fabricate(:queue_item, user: josias, position: 1)
			queue_item2 = Fabricate(:queue_item, user: josias, position: 2)
	...
	describe "POST update_queue" do
		...
		context "with valid inputs" do
			...
			let(:queue_item1) { Fabricate(:queue_item, user: josias, position: 1, video: video) }
			let(:queue_item2) { Fabricate(:queue_item, user: josias, position: 2, video: video) }
			
			before { set_current_user(josias) }
			...
			it "reorders the queue items" do
				post :update_queue, queue_items: [{id: queue_item1.id, position: 2}, {id: queue_item2.id, position: 1}]
				expect(josias.queue_items).to eq([queue_item2, queue_item1])
			end
			
			it "normalizes the position numbers" do
				post :update_queue, queue_items: [{id: queue_item1.id, position: 3}, {id: queue_item2.id, position: 2}]
				expect(josias.queue_items.map(&:position)).to eq([1, 2])
			end
		end
		...
		context "with queue items that do not belong to the current user" do
			it "does not change the queue items" do
				...
				queue_item2 = Fabricate(:queue_item, user: josias, position: 2, video: video)
				post :update_queue, queue_items: [{id: queue_item1.id, position: 3}, {id: queue_item2.id, position: 2}]
				expect(queue_item1.reload.position).to eq(1)
			end
		end
