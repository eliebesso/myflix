...
describe RelationshipsController do
	describe "GET index" do
		it "sets @relationships to the current user's following relationships" do
			...
			bob = Fabricate(:user)
			relationship = Fabricate(:relationship, follower: josias, leader: bob)
			get :index
	...
	describe "POST create" do
		...
		it "creates a relationship that the current user follows the leader" do
			...
			set_current_user(josias)
			post :create, leader_id: bob.id
			expect(josias.following_relationships.first.leader).to eq(bob)
		end
		...
		it "does not allow one to follow themselves" do
			josias = Fabricate(:user)
			set_current_user(josias)
			post :create, leader_id: josias.id
			expect(Relationship.count).to eq(0)
		end
	end
end
